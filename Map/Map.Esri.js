...
define([
  ...
  dojo.addOnLoad(function() {
    ...
    */
    ...
    /* wax.esri */
    wax.esri=wax.esri||{};wax.esri.interaction=function(){function b(){g=!0}var g=!1,e,a,c;return wax.interaction().attach(function(d){if(!arguments.length)return a;a=d;c=[dojo.connect(a,"onExtentChange",b),dojo.connect(a,"onUpdateEnd",b),dojo.connect(a,"onReposition",b)]}).detach(function(){for(var a=0;a<c.length;a++)dojo.disconnect(c[a])}).parent(function(){return a.root}).grid(function(){if(g||!e){e=[];for(var d=0;d<a.layerIds.length;d++)for(var b=a.getLayer(a.layerIds[d])._div.getElementsByTagName("img"),f=0;f<b.length;f++){var c=wax.u.offset(b[f]);e.push([c.top,c.left,b[f]])}}return e})};dojo.declare("wax.esri.connector",esri.layers.TiledMapServiceLayer,{constructor:function(a){a=a||{};this.options={tiles:a.tiles,minzoom:a.minzoom||0,maxzoom:a.maxzoom||22};this.spatialReference=new esri.SpatialReference({wkid:3857});this.fullExtent=this.initialExtent=new esri.geometry.Extent(-2.0037508342789E7,-2.0037508342789E7,2.0037508342789E7,2.0037508342789E7,this.spatialReference);for(var a=[],b=this.options.minzoom;b<=this.options.maxzoom;b++)a.push({level:b,scale:5.91657527591555E8/Math.pow(2,b),resolution:156543.033928/Math.pow(2,b)});this.tileInfo=new esri.layers.TileInfo({spatialReference:{wkid:"3857"},rows:256,cols:256,origin:{x:-2.0037508342789E7,y:2.0037508342789E7},lods:a});this.loaded=!0;this.onLoad(this)},getTileUrl:function(a,b,c){return this.options.tiles[parseInt(Math.pow(2,a)*b+c,10)%this.options.tiles.length].replace("{z}",a).replace("{x}",c).replace("{y}",b)}});
